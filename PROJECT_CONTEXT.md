...
- **Tablas activas**:
  - `habits`: id, user_id, title, description, icon, type, daily_goal, initial_date, created_at, updated_at, synced
  - `habit_progress`: id, habit_id (FK→habits CASCADE), date, daily_goal, daily_counter, synced. UNIQUE(habit_id, date).
  - `users`: gestionada por Supabase Auth
- **Índices**: `idx_habits_user_initial_date(user_id, initial_date DESC)`, `idx_habit_progress_habit_id`, `idx_habit_progress_date`
...
- **Providers activos**:
  - `HabitsProvider` — lista de hábitos, paginación (pageSize=10), CRUD, progreso
  - `NewHabitProvider` — estado de formulario de creación
...
| `flutter_svg` | 2.2.1 | Íconos SVG personalizados |
...
| `lucide_icons` | 0.257.0 | Iconografía principal |
